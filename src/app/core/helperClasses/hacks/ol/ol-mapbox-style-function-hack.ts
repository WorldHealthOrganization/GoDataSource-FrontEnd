// ~ CLONE OF node_modules\ol-mapbox-style\dist\stylefunction.js
...
import { deg2rad, defaultResolutions, getZoomForResolution, wrapText, applyLetterSpacing, createCanvas } from './ol-mapbox-style-function-util-hack';
import * as olMapboxStyle from './ol-mapbox-style-spec-hack.js';
 ...
 */
const isFunction = olMapboxStyle.function.isFunction;
const convertFunction = olMapboxStyle.function.convertFunction;
const isExpression = olMapboxStyle.expression.isExpression;
const createPropertyExpression = olMapboxStyle.expression.createPropertyExpression;
const types = {
    'Point': 1,
    'MultiPoint': 1,
    'LineString': 2,
...
export function getValue(layer, layoutOrPaint, property, zoom, feature) {
    ...
    if (!functions[property]) {
        let value_1 = (layer[layoutOrPaint] || emptyObj)[property];
        const propertySpec = olMapboxStyle.latest[layoutOrPaint + '_' + layer.type][property];
        if (value_1 === undefined) {
            value_1 = propertySpec.default;
        }
        let isExpr = isExpression((value_1));
        if (!isExpr && isFunction(value_1)) {
            value_1 = convertFunction(value_1, propertySpec);
            isExpr = true;
        }
        if (isExpr) {
            const compiledExpression = expressionData(value_1, propertySpec);
            functions[property] = compiledExpression.evaluate.bind(compiledExpression);
        } else {
            if (propertySpec.type === 'color') {
                value_1 = olMapboxStyle.Color.parse(value_1);
            }
...
function evaluateFilter(layerId, filter, feature, zoom) {
    if (!(layerId in filterCache)) {
        filterCache[layerId] = olMapboxStyle.featureFilter(filter).filter;
    }
    zoomObj.zoom = zoom;
...
export default function (olLayer, glStyle, source, resolutions, spriteData, spriteImageUrl, getFonts) {
    ...
    if (spriteImageUrl) {
        ...
        } else if (typeof WorkerGlobalScope !== 'undefined' && self instanceof WorkerGlobalScope) { // tslint:disable-line
            const worker: any = /** @type {*} */ (self);
    ...
    for (let i = 0, ii = allLayers.length; i < ii; ++i) {
        ...
        if (typeof source === 'string' && layer.source === source ||
            ...
            if (!mapboxSource) {
                ...
                const source_1 = glStyle.sources[mapboxSource];
                if (!source_1) {
                    throw new Error('Source "' + mapboxSource + '" is not defined');
                }
                const type = source_1.type;
    ...
    const styleFunction = function (feature, resolution) {
        ...
        const _loop_1 = function (i, ii) {
            ...
            if (!filter || evaluateFilter(layerId, filter, f, zoom)) {
                ...
                let color = void 0, opacity = void 0, fill = void 0, stroke = void 0, strokeColor = void 0, style = void 0;
                ...
                if (type === 3 && (layer.type === 'fill' || layer.type === 'fill-extrusion')) {
                    ...
                    if (layer.type + '-pattern' in paint) {
                        ...
                        if (fillIcon) {
                            const icon_1 = typeof fillIcon === 'string'
                            ...
                            if (spriteImage && spriteData && spriteData[icon_1]) {
                                ...
                                const icon_cache_key = icon_1 + '.' + opacity;
                                let pattern = patternCache[icon_cache_key];
                                if (!pattern) {
                                    const spriteImageData = spriteData[icon_1];
                ...
                if (type !== 1 && layer.type === 'line') {
                    color = !('line-pattern' in paint) && 'line-color' in paint ?
                        ...
                        undefined;
                    const width_1 = getValue(layer, 'paint', 'line-width', zoom, f);
                    if (color && width_1 > 0) {
                        ++stylesLength;
                        style = styles[stylesLength];
                ...
                let icon = void 0, iconImg = void 0, skipLabel = void 0;
                                ...
                                if (geom.getFlatMidpoint || geom.getFlatMidpoints) {
                                    ...
                                    if (size > 150) {
                                        ...
                                        if (placement === 'line' && iconRotationAlignment === 'map') {
                                            ...
                                            for (let i_1 = 0, ii_1 = coordinates.length - stride; i_1 < ii_1; i_1 += stride) {
                                                const x1 = coordinates[i_1];
                                                const y1 = coordinates[i_1 + 1];
                                                const x2 = coordinates[i_1 + stride];
                                                const y2 = coordinates[i_1 + stride + 1];
